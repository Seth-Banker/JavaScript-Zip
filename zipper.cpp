...
#else
  if (std::fseek(fp, 0, SEEK_END) != 0) { std::fclose(fp); throw std::runtime_error("seek fail"); }
  long l = std::ftell(fp);
  if (l < 0) { std::fclose(fp); throw std::runtime_error("tell fail"); }
  if (std::fseek(fp, 0, SEEK_SET) != 0) { std::fclose(fp); throw std::runtime_error("seek fail"); }
  long long len = l;
#endif
